...
class UserGalleryViewController: UIViewController, InfiniteLoadingViewControllerDelegate, MenuDelegate {
    ...
    private var streamViewController : GalleryStreamViewController!
    ...
    private func createStreamViewController(_ style : StreamDisplayStyle, animated : Bool) {
        let stream = StorageService.sharedInstance.getGalleryPhotoStream(userId: user.userId, galleryId: gallery.galleryId)
        streamViewController = GalleryStreamViewController(gallery : gallery, stream: stream, style : style)
